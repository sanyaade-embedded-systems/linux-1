...
static int usb_ehci_bus_suspend(struct usb_hcd *hcd)
{
	...
	if (!cpu_is_omap3517() && !cpu_is_omap3505() && gb_omap->phy_reset) {
		if (gb_omap->port_mode[0] == EHCI_HCD_OMAP_MODE_PHY &&
			gpio_is_valid(gb_omap->reset_gpio_port[0])) {
			gpio_set_value(gb_omap->reset_gpio_port[0], 0);
		}
		if (gb_omap->port_mode[1] == EHCI_HCD_OMAP_MODE_PHY &&
			gpio_is_valid(gb_omap->reset_gpio_port[1])) {
			gpio_set_value(gb_omap->reset_gpio_port[1], 0);
		}
	}
	return ret;
}
static int usb_ehci_bus_resume(struct usb_hcd *hcd)
{
	...
	if (!cpu_is_omap3517() && !cpu_is_omap3505() && gb_omap->phy_reset) {
		if (gb_omap->port_mode[0] == EHCI_HCD_OMAP_MODE_PHY &&
			...
			gpio_set_value(gb_omap->reset_gpio_port[0], 0);
			udelay(30);
			gpio_set_value(gb_omap->reset_gpio_port[0], 1);
		}
		if (gb_omap->port_mode[1] == EHCI_HCD_OMAP_MODE_PHY &&
				gpio_is_valid(gb_omap->reset_gpio_port[1])) {
			gpio_set_value(gb_omap->reset_gpio_port[1], 0);
			udelay(30);
			gpio_set_value(gb_omap->reset_gpio_port[1], 1);
		}
	}
	return ret;
}
