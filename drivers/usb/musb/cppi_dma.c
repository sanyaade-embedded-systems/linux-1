...
static void cppi_dump_rx(int level, struct cppi_channel *c, const char *tag)
{
	...
	DBG(level, "RX DMA%d%s: %d left, csr %04x, "
		...
		musb_readl(&rx->rx_skipbytes, 0),	/* buf offset */
		musb_readl(&rx->rx_head, 0),		/* head ptr */
		musb_readl(&rx->rx_sop, 0),		/* sop bd */
		musb_readl(&rx->rx_current, 0),		/* current bd */
		musb_readl(&rx->rx_buf_current, 0),	/* current buf */
		musb_readl(&rx->rx_len_len, 0),		/* pkt len */
		musb_readl(&rx->rx_cnt_cnt, 0),		/* byte cnt */
		musb_readl(&rx->rx_complete, 0)		/* completion */
		);
}
...
static void cppi_dump_tx(int level, struct cppi_channel *c, const char *tag)
{
	...
	DBG(level, "TX DMA%d%s: csr %04x, "
		...
		musb_readl(&tx->tx_head, 0),		/* head ptr */
		musb_readl(&tx->tx_buf, 0),		/* sop bd */
		musb_readl(&tx->tx_current, 0),		/* current bd */
		musb_readl(&tx->tx_buf_current, 0),	/* buf offset */
		musb_readl(&tx->tx_info, 0),		/* flags */
		musb_readl(&tx->tx_rem_len, 0),		/* len */
		/* dummy/unused word 6 */
		musb_readl(&tx->tx_complete, 0)		/* completion */
		);
}
...
cppi_next_tx_segment(struct musb *musb, struct cppi_channel *tx, int rndis)
{
	...
	/* Disable RNDIS on both Tx & Rx.  Enable when the applications to be
	 * supported are only short packet aware and a single URB length does
	 * not exceed 64K.
...
static int cppi_rx_scan(struct cppi *cppi, unsigned ch, u8 abort)
{
	...
	} while (musb_readl(&state->rx_complete, 0) != safe2ack);
...
void cppi_completion(struct musb *musb, u32 rx, u32 tx)
{
	...
	for (channum = 0; rx; rx = rx >> 1, channum++) {
		...
		if (rx & 1) {
			...
			if ((rx_ch->actuallen != rx_ch->buf_len)
			    && !(rx_ch->reqcomplete)) {
				cppi_next_rx_segment(musb, rx_ch,
						     rx_ch->rxmode);
				continue;
			}
			...
			rx_ch->channel.actual_len = rx_ch->actuallen;
...
{
	struct cppi *cppi = container_of(c, struct cppi, controller);;
...
void cppi_tx_completion_backoff(struct musb *musb)
{
	...
	for (channum = 0; channum < 4; channum++) {
		...
		if (txchannel->tx_comp_guard) {
			if (!txchannel->hw_ep) {
				txchannel->tx_comp_guard = 0;
				continue;
			}

                        epio = txchannel->hw_ep->regs;
			...
			if (csr & (MUSB_TXCSR_TXPKTRDY |
				...
				txchannel->tx_comp_guard = 0;
				txchannel->hw_ep->dma_completed = 1;
				musb_dma_completion(musb, txchannel->hw_ep->epnum,
							1);
				txchannel->hw_ep->dma_completed = 0;
                        }
		}
	}
