...
static int taal_dcs_read_1(u8 dcs_cmd, u8 *data)
{
	...
	r = dsi2_vc_dcs_read(TCH, dcs_cmd, buf, 1);
...
{
	return dsi2_vc_dcs_write(TCH, &dcs_cmd, 1);
}
...
static int taal_dcs_write_1(u8 dcs_cmd, u8 param)
{
	...
	buf[0] = dcs_cmd;
	buf[1] = param;
	return dsi2_vc_dcs_write(TCH, buf, 2);
}
...
static int taal_sleep_in(struct taal_data *td)
{
	...
	cmd = DCS_SLEEP_IN;
	r = dsi2_vc_dcs_write_nosync(TCH, &cmd, 1);
	if (r)
		return r;
		...
		u16 x, u16 y, u16 w, u16 h)
{
	...
	r = dsi2_vc_set_max_rx_packet_size(TCH, plen);
	if (r)
		return r;
	...
	while (buf_used < size) {
		...
		r = dsi2_vc_dcs_read(TCH, dcs_cmd,
...
err:
	dsi2_vc_set_max_rx_packet_size(TCH, 1);
