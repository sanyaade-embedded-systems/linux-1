...
static int zoom2_spi_probe(struct spi_device *spi)
{
	...
	omap_dss_register_driver(&nec_driver);
	return 0;
}

static int zoom2_spi_remove(struct spi_device *spi)
{
	omap_dss_unregister_driver(&nec_driver);
...
static int __init nec_lcd_init(void)
{
	return spi_register_driver(&zoom2_spi_driver);
}

static void __exit nec_lcd_exit(void)
{
	return spi_unregister_driver(&zoom2_spi_driver);
}


module_init(nec_lcd_init);
module_exit(nec_lcd_exit);
MODULE_LICENSE("GPL");
