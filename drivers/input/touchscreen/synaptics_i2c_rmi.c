...
err_page_select_failed:
	...
	if (ret < 0)
		printk(KERN_ERR "synaptics_ts_resume: "
...
static void synaptics_ts_work_func(struct work_struct *work)
{
	...
	for (i = 0; i < ((ts->use_irq && !bad_data) ? 1 : 10); i++) {
		...
		if (ret < 0) {
			printk(KERN_ERR "synaptics_ts_work_func: "
					"i2c_transfer failed\n");
			bad_data = 1;
		} else {
			...
			if ((buf[buf_len - 1] & 1) == 0) {
				...
				int f, a;
				int base;
				/* int x = buf[3] | (uint16_t)(buf[2] & 0x1f)
									<< 8; */
				/* int y = buf[5] | (uint16_t)(buf[4] & 0x1f)
									<< 8; */
				int z = buf[1];
				int w = buf[0] >> 4;
				int finger = buf[0] & 7;

				/* int x2 = buf[3+6] | (uint16_t)(buf[2+6] & 0x1f)
						<< 8; */
				/* int y2 = buf[5+6] | (uint16_t)(buf[4+6] & 0x1f)
						<< 8; */
				/* int z2 = buf[1+6]; */
				/* int w2 = buf[0+6] >> 4; */
				...
				/* printk("x %4d, y %4d, z %3d, w %2d,
					F %d, 2nd: x %4d, y %4d, z %3d,
					w %2d, F %d, dx %4d, dy %4d\n", */
				/*	x, y, z, w, finger, */
				/*	x2, y2, z2, w2, finger2, */
				/*	dx, dy); */
				...
				if (finger2_pressed) {
					...
					input_report_abs(ts->input_dev,
							ABS_MT_WIDTH_MAJOR, w);
					input_report_abs(ts->input_dev,
							ABS_MT_POSITION_X, pos[1][0]);
					input_report_abs(ts->input_dev,
							ABS_MT_POSITION_Y, pos[1][1]);
					input_mt_sync(ts->input_dev);
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->irq) {
		...
		if (ret == 0) {
			/* enable abs int */
			ret = i2c_smbus_write_byte_data(ts->client, 0xf1, 0x01);
			if (ret)
...
static int synaptics_ts_resume(struct i2c_client *client)
{
	...
	if (!ts->use_irq)
		...
		/* enable abs int */
		i2c_smbus_write_byte_data(ts->client, 0xf1, 0x01);
...
{
	#warning TODO : Fix checkpatch errors for Synaptics touch screen driver.
