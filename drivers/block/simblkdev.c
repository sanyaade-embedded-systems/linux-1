                     ...
                     struct request *       req)
{
    ...
    if (write) {
        if (   llsbd_writeblk (dev->ioaddr, dev->fd, offset, (void *) phys, nbytes)
            != nbytes) {
            printk (KERN_NOTICE "SimBlkDev: Low level write error (%ld 0x%08x %ld)\n",
                    offset, (unsigned int) phys, (long int) nbytes);
            return;
        }
    }
    else {
        if (   llsbd_readblk (dev->ioaddr, dev->fd, offset, (void *) phys, nbytes)
            != nbytes) {
            printk (KERN_NOTICE "SimBlkDev: Low level read error (%ld 0x%08x %ld)\n",
                    offset, (unsigned int) phys, (long int) nbytes);
            return;
        }
 ...
 */
    ...
    if (SimBlkDev_queue == NULL)
        goto out;
    blk_queue_hardsect_size (SimBlkDev_queue, SECTOR_SIZE);
 ...
 */
    ...
    if (major_num < 0) {
        printk(KERN_WARNING "SimBlkDev: unable to get major number %d\n", SBD_MAJOR);
        goto out;
    }
    else {
 ...
 */
    ...
    if (simBlkDev.fd < 0) {
        printk(KERN_WARNING "SimBlkDev: unable to start the low level emulation\n");
        goto out_unregister;
    }
 ...
 */
    ...
    if (simBlkDev.size <= 0) {
        ...
        printk ("<1>Using c:\\temp\\hd.img of size %ld\n", simBlkDev.size);
    }
