...
static int __init omap_nand_init(void)
{
	...
	if ((1 == (use_dma | use_interrupt)) && (0 == use_prefetch)) {
		printk(KERN_INFO"Wrong parameters: Niether of 'dma' or 'irq' "
				"can used without 'use_prefetch' selected. "
				"Prefetch will not be used in any mode: "
				"poll, mpu or dma\n");
	}
	if (1 == use_prefetch) {
		if (1 == use_interrupt && 1 == use_dma)
			printk(KERN_INFO"Wrong parameters: Both 'dma' and "
					"'irq' modes can not be used "
					"togather. It has to be selected at "
					"compile time and same will be used "
					"\n");
	}
