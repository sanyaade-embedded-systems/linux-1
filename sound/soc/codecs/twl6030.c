...
void twl6030_naudint_work(struct work_struct *work)
{
	...
	priv = setup->codec->private_data;

	twl_i2c_read_u8(TWL6030_MODULE_AUDIO, &intid, TWL6030_REG_INTID);

	switch (intid) {
	case TWL6030_THINT:
		printk(KERN_WARNING "TWL6030: Die temperature over-limit "
			"detection\n");
		break;
	...
	case TWL6030_HOOKINT:
		break;
	case TWL6030_HFINT:
		printk(KERN_WARNING "TWL6030: Left or right hands-free drivers"
			"over current detection\n");
		break;
	case TWL6030_VIBINT:
		printk(KERN_WARNING "TWL6030: Left or right vibrator drivers "
			"over current detection\n");
		break;
	case TWL6030_READYINT:
		...
		break;
	default:
		printk(KERN_ERR "TWL6030: unknown audio interrupt\n");
	}
				...
				unsigned int fmt)
{
	...
	if (format != SND_SOC_DAIFMT_TI_PDM)
		return -EINVAL;
...
static int twl6030_init(struct snd_soc_device *socdev)
{
	...
	printk(KERN_INFO "TWL6030 Audio Codec init \n");
	...
	if (ret < 0) {
		printk(KERN_ERR "twl6030: failed to create pcms\n");
		goto pcm_err;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "twl6030: failed to register card\n");
		goto card_err;
	}
...
static int twl6030_remove(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "TWL6030 Audio Codec remove\n");
	twl6030_set_bias_level(codec, SND_SOC_BIAS_OFF);
	snd_soc_free_pcms(socdev);
	snd_soc_dapm_free(socdev);
