 ...
 * ABE_API.H
 *
 * 'ABE_API.H' all the APIs used by the Audio driver
 *
 * @path 
 * @rev     01.00
 *! 
 ...
 *
 *  Parameter  : No parameter
 * @param        dma : output pointer to the DMA iteration and data destination 
 * pointer 
 * @pre          no pre-condition 
 * @post         
 * @return       error code 
 ...
 *
 *  Parameter  : 
 *      use-case-ID : "LP player", "voice-call" use-cases as defined in the 
 *      paragraph 
 *      "programming use-case sequences"
 *
 *  Operations : 
 *      private API used during development. Loads all the necessary parameters 
 *      and data patterns to allow a stand-alone functional test without the need
 *      of. 
 *
 *  Parameter  : No parameter
 * @param        dma : output pointer to the DMA iteration and data destination 
 ...
 *      Some IRQ source are originated for the delivery of "end of time sequenced
 *      tasks" notifications, some are originated from the Ping-Pong protocols, 
 *      some are generated from the embedded debugger when the firmware stops 
 *      on programmable break-points, etc …
 *
 * @param        dma : output pointer to the DMA iteration and data destination 
 * 		pointer 
 * @pre          no pre-condition 
...
/*
 * abe_event_generator_switch() description for void abe_event_generator_switch().  
 *
 *  Operations : 
 *      load the AESS event generator hardware source. Loads the firmware 
 *      parameters accordingly. Indicates to the FW which data stream is the most
 *      important to preserve in case all the streams are asynchronous. If the 
 *      parameter is "default", let the HAL decide which Event source is the best
 *      appropriate based on the opened ports.
 *
 * @param        e: Event Generation Counter, McPDM, DMIC or default.
 * @param        p: pointer to the main port
 * @pre          no pre-condition 
 * @post         
 * @return       error code
 * @see           
...
/*
 * abe_unplug_subroutine() description for void abe_unplug_subroutine().  
 *
 * Parameter :
 *  Id: index of the call-back to be removed
 *
 *  Operations : 
 *      unregister a  subroutine used for call-back.
 * @param        
 * @pre          no pre-condition 
 * @post         
 * @return       error code
...
/*
 * abe_set_analog_control() description for abe_set_analog_control().  
 *
 *  Parameter  : 
 ...
 *
 *  Operations : 
 *      returns to the data pointer the gain value of the phoenix headset in case
 *      the dynamic extension is activated.
 * @param        
 * @pre          no pre-condition 
 * @post         
 * @return       error code
 ...
 *      headset port. 
 *      This feature enables dynamic range extension by increasing the digital 
 *      gains and lowering the analog gains. This API is preferably called at
 *      reset time.
 *
 * @param        
 * @pre          no pre-condition 
 ...
 *
 *  Operations : 
 *      disables the audio engine capability to change the analog gain of Phoenix
 *      heads
 *
 * @param        
 * @pre          no pre-condition 
 ...
 *      scheduling task loops (250us / 272us with respectively 48kHz / 44.1kHz
 *      on Phoenix);. Translates this data 
 ...
 *      sampling frequency (2.4MHz or 3.84MHz);. This table compensates the DMIC
 *      decimator roll-off at 20kHz. The default table is loaded with the DMIC
 *      2.4MHz recommended configuration.
 *
 *  Return value : 
 *      None.
 */
 ...
 *
 *  Operations : 
 *      writes the corresponding port descriptor in the AE memory spaces. The
 *       ATC DMEM descriptors 
 *      are initialized. 
 ...
 *      None.
 */
void abe_write_port_descriptor (abe_port_id id, abe_data_format_t *f,
					abe_port_protocol_t *p, abe_dma_t *dma);
 ...
 *
 *  Return value : 
 *      error code in case the Port_id is not compatible with the current 
 *       OPP value
 */
void abe_read_port_descriptor (abe_port_id id, abe_data_format_t *f,
...
void abe_write_port_gain (abe_port_id port, abe_gain_port_t *abe_dig_gain_port);
 ...
 *      the gains of the analog part, else only the digital part is taken into 
 *      account.
 *
 *  Return value : 
 *      None.
 ...
 *      corresponds to "no equalizer feature". Loading all coefficients with
 *      zeroes disables the feature.
 *
 *  Return value : 
 *      None.
 */
 ...
 *      gain in mute state". A mixer is disabled with a network 
 ...
 *
 *  Operations : 
 *      Load the EANC coefficients in FW memory. This API can be called when
 *      EANC is not activated. After reloading the firmware the default
 *      coefficients corresponds to "no EANC activated". Loading the
 *      coefficients value with zero disables the feature.
 *
 *  Return value : 
 *      None.
 */
 ...
 *      individually stored in an intermediate table of 10 elements. The 
 *      intermediate table is used to route the samples to 
 *      three directions : REC1 mixer, 2 EANC DMIC source of filtering and MM 
 *      recording audio path. 
 *      For example, a use case consisting in AMIC used for uplink voice 
 *      communication, DMIC 0,1,2,3 used for multimedia recording, , DMIC 5 used
 *       for EANC filter, DMIC 4 used for the feedback channel, 
 ...
 *      timestamps, and store 
 *      the pairs in linear addressing to the parameter pointer. Stops the copy
 *      when the max 
