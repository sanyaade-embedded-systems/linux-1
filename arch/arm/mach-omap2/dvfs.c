		...
		struct omap_vdd_dvfs_info *tdvfs_info)
{
	...
	if (IS_ERR_OR_NULL(voltdm) || IS_ERR_OR_NULL(voltdm)) {
...
fail:
	/* Set me back up for old domain maps */
	_dep_scan_domains(target_dev, vdd, curr_volt);
	_dep_scale_domains(target_dev, vdd);
	voltdm_scale(voltdm, curr_volt);
out:
			...
			unsigned long rate)
{
	...
	if (IS_ERR_OR_NULL(od) || IS_ERR_OR_NULL(pdev)) {
		pr_err("%s: od or pdev is null!\n", __func__);
		return -EINVAL;
	}
 ...
 * Returns 0 on success.
 */
int __init omap_dvfs_register_device(struct omap_hwmod *oh, struct device *dev)
{
	struct omap_vdd_dev_list *temp_dev;
	struct omap_vdd_dvfs_info *dvfs_info;
	struct powerdomain *pdm;
	struct voltagedomain *voltdm;
	struct clk *clk = NULL;
	int ret = 0;

	pdm = omap_hwmod_get_pwrdm(oh);
	if (IS_ERR_OR_NULL(pdm)) {
		dev_err(dev, "%s: No power domain for hwmod!\n", __func__);
		return -EINVAL;
	}
	voltdm = pwrdm_get_voltdm(pdm);
	if (IS_ERR_OR_NULL(voltdm) || !voltdm->name) {
		dev_err(dev, "%s: Bad voltdm pointer!\n", __func__);
		return -EINVAL;
	}
	if (IS_ERR_OR_NULL(oh->main_clk)) {
		dev_err(dev, "%s: Bad clk name!\n", __func__);
		if (IS_ERR_OR_NULL(oh->_clk)) {
			dev_err(dev, "%s: Bad clk as well.. giving up!\n",
					__func__);
			return -EINVAL;
		}
		clk = oh->_clk;
	...
	if (!dvfs_info) {
		...
		dvfs_info->voltdm = voltdm_lookup(voltdm->name);
		if (!dvfs_info->voltdm) {
