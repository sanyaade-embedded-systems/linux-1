	...
	*/
	...
	if (IS_PM34XX_ERRATA(PER_WAKEUP_ERRATA_XYZ)) {
		...
		if ((coreprev_state == 0x3) && (perprev_state == 0x0)) {
				...
				pr_err("Hit the UART h/w errata..reset??\n");
				/*
				 * We dont seem to have a real recovery
				 * other than may be reset the device??
				 */
		}
	}
...
static void __init prcm_setup_regs(void)
{
	...
	if (cpu_is_omap3630()) {
		cm_clksel1_iva2 =
			(cm_clksel1_iva2 & ~(OMAP3430_IVA2_CLK_SRC_MASK)) |
			(0x2 << OMAP3430_IVA2_CLK_SRC_SHIFT);
		cm_clksel1_mpu =
			(cm_clksel1_mpu & ~(OMAP3430_MPU_CLK_SRC_MASK)) |
...
static void sr_class1p5_reset_calib(void)
{
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < num_mpu_opps; i++)
		if (mpu_opps[i].rate)
			tbuf += sprintf(tbuf, "mpu %d:\t0x%0ld\t\t0x%02x\n", i,
					mpu_opps[i].u_volt,
					mpu_opps[i].sr_adjust_vsel);
	for (i = 0; i < num_l3_opps; i++)
		if (l3_opps[i].rate)
			tbuf += sprintf(tbuf, "l3 %d:\t0x%0ld\t\t0x%02x\n", i,
					l3_opps[i].u_volt,
